...
class DDIM(nn.Module):
    ...
    def __init__(self, input_channels=1):
        ...
        ## noise variance during sampling
        # self.sigma = torch.sqrt(self.beta)
        self.sigma = torch.zeros_like(self.beta)
    ...
    def next(self, x, t):
        ...
        # et = z
        # x0_pred = x - (1 - model.alpha[t]**0.5 * et)
        # x0_pred /= model.alpha[t]**0.5

        # xt_dir = (1 - model.alpha[t-1] - model.sigma[t]**2)**0.5 * e_theta if t > 0 else 0
        # noise = model.sigma[t] * et
        # # noise = 0

        # ## x_t-1
        # x = model.alpha[t-1]**0.5 * x0_pred + xt_dir + noise

        ## https://chatgpt.com/share/67cfdc2b-3a4c-800c-a5d5-518e2b10705d
        # alpha_t = model.alpha_bar[t]
        # alpha_next = model.alpha_bar[t-1]
        # noise_pred = z
